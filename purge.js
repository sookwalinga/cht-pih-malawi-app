module.exports = {
  ...
  fn: (userCtx, reports, messages) => {
    ...
    const reportsToPurge = reports
      .filter((r) => {
        if (
          userCtx.roles.includes('supervisor') &&
          !SCHW_WORKFLOWS.includes(r.form)
        ) {
          return true
        }
        const purgeThreshold = ['pregnancy', 'pregnancy_follow_up'].includes(
          r.form
        )
          ? 12
          : 6
    ...
    const messagesToPurge = messages
      .filter((m) => m.reported_date <= monthsAgo(6))
      .map((m) => m._id)
